...
# 1. Setup - MUST MATCH YOUR COLLECTION DATA
DATA_PATH = "data"
# Ensure these are exactly the signs you recorded
...
for sign in signs:
    for sequence in range(no_sequences):
        ...
        for frame_num in range(sequence_length):
            ...
            img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE) # Faster & focuses on shape
...
X = np.expand_dims(X, axis=-1) / 255.0 # Normalize pixels 0-1
y = to_categorical(labels).astype(int)

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1)

# 3. The "Pure CNN" Architecture
model = Sequential([
    # TimeDistributed allows the CNN to see every frame individually
    ...
    # The LSTM looks at the CHANGE between frames (The Motion)
    LSTM(128, return_sequences=True, activation='tanh'),
    LSTM(64, return_sequences=False, activation='tanh'),
    
    # Decision Making
    Dense(64, activation='relu'),
    Dropout(0.5), # Prevents memorizing the background
    Dense(len(signs), activation='softmax')
])

model.compile(optimizer='Adam', loss='categorical_crossentropy', metrics=['categorical_accuracy'])

# 4. Training Callbacks (Prevents Overheating/Overfitting)
